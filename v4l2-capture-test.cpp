...
void parse_cli_or_die(int argc, const char** argv) {
  ...
  // popt options table; add more options here as needed
  ...
  // If --devices was provided, split it on commas
  if (opt_devices && *opt_devices) {
    ...
    // Consume any trailing non-option args, but ignore them if --devices was used
    const char* extra = nullptr;
    while ((extra = poptGetArg(pc)) != nullptr) {
      fprintf(stderr, "[main] Warning: ignoring extra device '%s' because --devices was specified\n", extra);
    }
  } else {
    // Fallback: collect remaining non-option arguments as device names
  ...
  if (devNames.size() == 1) {
    ...
    fprintf(stderr, "  %s [options] --devices=</dev/video0>[,/dev/video1]\n", argv[0]);
    fprintf(stderr, "  %s [options] </dev/video0> [/dev/video1]\n", argv[0]);
  ...
  // Optional: echo parsed settings
  fprintf(stderr, "[main] Parsed options: --fps=%.3f, --devices=%s%s%s\n",
          allDevicesTargetFramerate,
          devNames[0].c_str(),
          isDualInput ? "," : "",
  ...
  if (devInfos->isTC358743) {
    ...
    if (ret >= 0) {
      ...
      if (ret < 0) {
        ...
        devInfos->targetFrameDelayMicros = (1000000 / devInfos->framerate) * devInfos->framerateDivisor;
        devInfos->targetFrameDelayMillis = (1000 / devInfos->framerate) * devInfos->framerateDivisor;
        ...
        int rawOutputThroughput = ((float)((float)(devInfos->framerate * devInfos->startingSize) / 125000.0F)) / devInfos->framerateDivisor; // Measured in megabits/sec based on output framerate
...
int main(const int argc, char** argv) {
  ...
  while (shouldLoop) {
    ...
    background_task_cap_main = std::async(std::launch::async, get_frame, buffersMain, devInfoMain);
    background_task_cap_main.wait();
    fwrite(devInfoMain->outputFrame, sizeof(unsigned char), devInfoMain->startingSize, stdout);
    if (!devInfoMain->realAndTargetRatesMatch) usleep(devInfoMain->targetFrameDelayMicros - sw.elapsedMicros());
  }
  usleep(1000000);
  cleanup_vars();
  return 0;
