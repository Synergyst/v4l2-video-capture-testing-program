...
PRODUCT="CM4"
#PRODUCT="RPi0"
#echo "CM4 USB Device UVC" > $GADGET_CONFIGFS_ROOT/strings/0x409/product
...
create_uvc() {
        ...
        case "$PLAT" in
          test)
            ...
            ;;
          mjpeglowres)
            create_frame $FUNCTION 324 256 mjpeg m
          ;;
          mjpeg)
            create_frame $FUNCTION 1280 720 mjpeg m
          ;;
          yuyvlowres)
            create_frame $FUNCTION 324 256 uncompressed u
          ;;
          yuyvhighres)
        ...
        #echo 2560 > functions/$FUNCTION/streaming_maxpacket
...
case "$OP" in
    start)
        ...
        echo "Creating gadget directory cm4"
        mkdir -p $GADGET/cm4
