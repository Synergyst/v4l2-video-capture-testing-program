...
bool addKey(uint8_t k) {
  if (k == 0) return false;
  // already pressed?
  for (int i = 0; i < 6; i++) if (keys[i] == k) return false;
  // find empty slot
  for (int i = 0; i < 6; i++) {
    if (keys[i] == 0) { keys[i] = k; return true; }
  }
...
bool removeKey(uint8_t k) {
  ...
  bool removed = false;
  for (int i = 0; i < 6; i++) {
    if (keys[i] == k) { keys[i] = 0; removed = true; }
  }
  // compact
  if (removed) {
    uint8_t tmp[6] = {0,0,0,0,0,0};
    int j = 0;
    for (int i = 0; i < 6; i++) if (keys[i]) tmp[j++] = keys[i];
...
uint8_t codeToModifierMask(const String &code) {
  String c = unquote(code); String lc = c; lc.toLowerCase();
...
bool charToKeyCode(char ch, uint8_t &code, uint8_t &needShift) {
  code = 0; needShift = 0;
  // Letters
  if (ch >= 'a' && ch <= 'z') { code = KEY_A + (ch - 'a'); return true; }
  if (ch >= 'A' && ch <= 'Z') { code = KEY_A + (ch - 'A'); needShift = MODIFIERKEY_SHIFT; return true; }
  // Digits
  if (ch >= '1' && ch <= '9') { code = KEY_1 + (ch - '1'); return true; }
  if (ch == '0')              { code = KEY_0; return true; }
  // Space
  if (ch == ' ')              { code = KEY_SPACE; return true; }
  // Common US punctuation (unshifted)
  if (ch == '-') { code = KEY_MINUS; return true; }
  if (ch == '=') { code = KEY_EQUAL; return true; }
  if (ch == '[') { code = KEY_LEFT_BRACE; return true; }
  if (ch == ']') { code = KEY_RIGHT_BRACE; return true; }
  if (ch == '\\'){ code = KEY_BACKSLASH; return true; }
  if (ch == ';') { code = KEY_SEMICOLON; return true; }
  if (ch == '\''){ code = KEY_QUOTE; return true; }
  if (ch == ',') { code = KEY_COMMA; return true; }
  if (ch == '.') { code = KEY_PERIOD; return true; }
  if (ch == '/') { code = KEY_SLASH; return true; }
  if (ch == '`') { code = KEY_TILDE; return true; }
  // Shifted variants (examples)
  if (ch == '_') { code = KEY_MINUS; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '+') { code = KEY_EQUAL; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '{') { code = KEY_LEFT_BRACE; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '}') { code = KEY_RIGHT_BRACE; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '|') { code = KEY_BACKSLASH; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == ':') { code = KEY_SEMICOLON; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '"') { code = KEY_QUOTE; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '<') { code = KEY_COMMA; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '>') { code = KEY_PERIOD; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '?') { code = KEY_SLASH; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '~') { code = KEY_TILDE; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '!') { code = KEY_1; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '@') { code = KEY_2; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '#') { code = KEY_3; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '$') { code = KEY_4; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '%') { code = KEY_5; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '^') { code = KEY_6; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '&') { code = KEY_7; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '*') { code = KEY_8; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == '(') { code = KEY_9; needShift = MODIFIERKEY_SHIFT; return true; }
  if (ch == ')') { code = KEY_0; needShift = MODIFIERKEY_SHIFT; return true; }
  return false;
}
    ...
    if (k.length() == 1) {
      ...
      if (charToKeyCode((char)k.charAt(0), kc, needShift) && kc != 0) {
        if (isDown) {
          ...
          if (needShift && !(modMask & needShift)) { modMask |= needShift; addedShift = true; }
...
void loop() {
  ...
  String tok[MAX_TOK];
  int n = 0; bool inQ = false; String cur = "";
  for (unsigned int i = 0; i < line.length(); i++) {
    char ch = line.charAt(i);
    if (ch == '"') { inQ = !inQ; continue; }
    if (!inQ && isWhitespace(ch)) {
      if (cur.length() > 0 && n < MAX_TOK) { tok[n++] = cur; cur = ""; }
    } else {
      cur += ch;
    }
  }
  ...
  if (cmd == "MREL" && n >= 3) {
    ...
    Serial.print("RAW: ");
    for (int i = 1; i < n; i++) { Serial.print(tok[i]); if (i + 1 < n) Serial.print(' '); }
    Serial.println();
  } else {
