...
<style>
  ...
  #hud{position:fixed;left:8px;top:8px;right:auto;display:flex;gap:.5rem;align-items:center;font:14px system-ui;color:#fff;opacity:.9}
  ...
  #hud .dim{opacity:.7}
  #help{position:fixed;right:8px;top:8px;max-width:38ch;background:#111;border:1px solid #333;border-radius:8px;padding:.6rem .8rem;color:#ddd;font:13px system-ui}
...
(function(){
  ...
  function onWheel(ev) {
    if (!capturing) return;
    // Normalize: send pixel deltas (browser may report lines/pages)
  ...
  function onKey(ev, action) {
    if (!capturing) return;
    // Avoid spamming repeats on keydown; let remote OS handle repeat
    ...
    // Prevent browser shortcuts where possible
    ev.preventDefault();
  }
