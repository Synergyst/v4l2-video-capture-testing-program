...
struct CRTParams {
    ...
    size_t block_rows = 32;          // multithread chunk size (rows) - unused by CUDA path
};
...
public:
    ...
    void apply(const uint8_t* src_rgb24, std::vector<uint8_t>& dst);            // auto-advancing
...
private:
    ...
    int h_;
    CRTParams p_;
    ThreadPool pool_; // kept for API compatibility; not used by CUDA path
    std::mt19937 rng_;
    float phase60_ = 0.0f;
    float phaseH_ = 0.0f;
    float phaseV_ = 0.0f;
